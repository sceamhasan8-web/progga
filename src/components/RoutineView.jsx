...
export const RoutineTable = ({
  ...
  return (
    <div className="routine-table-container" style={{ width: '100%', overflowX: 'auto', background: TOKENS.paper, padding: 10, borderRadius: 12, boxShadow: '0 1px 2px rgba(28,35,51,0.06)', border: `1px solid ${TOKENS.line}` }}>
...
export const TeacherRoutine = ({ teacherName, routine = {}, onUpdate, readOnly = false, timeSlots = DEFAULT_TIME_SLOTS }) => {
  ...
  // Safe state update logic initializing array safely if gridRoutine[day] is empty/undefined
...
// ==========================================
// CLASS ROUTINE MANAGER COMPONENT (direct text inputs for all cells)
// ==========================================
export const ClassRoutineManager = ({
  ...
  // Real-time state update logic for ClassRoutineManager
  ...
  const handleDeleteGroup = useCallback(async (groupId, groupName) => {
    ...
    const ok = await confirm({
      ...
      message: `"${groupName}" গ্রুপটি মুছে ফেলতে চান? এই গ্রুপের পুরো রুটিন মুছে যাবে।`,
  ...
  }, [selectedClassId, selectedGroupId, onDeleteClassGroup, effectiveReadOnly, confirm]);
  ...
  const handleDeleteClass = useCallback(async (classId, className) => {
    if (!isAdmin) return;
    const ok = await confirm({
      title: 'Delete Class Confirmation',
      message: `"${className}" ক্লাসটি মুছে ফেলতে চান? এই ক্লাসের সব গ্রুপ ও রুটিন মুছে যাবে।`,
      confirmText: 'OK, Delete',
      cancelText: 'Cancel'
    });
    if (!ok) return;
  ...
  }, [selectedClassId, onDeleteClass, isAdmin, confirm]);
...
const SchoolRoutineManager = ({
  ...
  // Stateful localClasses initialized with props or localStorage backup
  ...
  // Stateful localTeacherRoutines initialized with props or localStorage backup
  ...
  useEffect(() => {
    if (tempTimeSlots.length === 0 && safeTimeSlots.length > 0) {
  ...
  // Immediate state update & localStorage backup for Teacher Routine
  ...
  // Deep state update for Class Routine matching target classId & groupId
  const handleSaveClassRoutine = useCallback((classId, groupId, updatedRoutine) => {
    setLocalClasses((prevClasses) => {
      const updatedClasses = (prevClasses || []).map((cls, classIdx) => {
        ...
        const currentClassName = String(cls.name || cls.className || '');
        if (currentClassId !== String(classId) && currentClassName !== String(classId)) {
          return cls;
        }
  ...
  const handleAddClass = useCallback((className, explicitBranchKey = null) => {
    setLocalClasses((prevClasses) => {
      const targetBranch = explicitBranchKey || getBranchKeyByClass(className) || 'primary';
      const newClass = {
        id: `class-${Date.now()}`,
        className: className,
    ...
    if (onAddClassProp) onAddClassProp(className, explicitBranchKey);
  }, [onAddClassProp]);
  ...
  // Comprehensive Firestore & localStorage Save Handler for both tabs
  ...
  return (
    <div className="routine-root routine-main-container" style={{ maxWidth: 1180, margin: '0 auto', padding: 20, background: TOKENS.parchment, minHeight: '100vh', display: 'flex', flexDirection: 'column', flex: 1, width: '100%' }}>
      <style>{ROUTINE_STYLES}</style>
